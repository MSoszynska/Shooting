def refine(
    ...
    fluid_residuals = [abs(x[0]) + abs(x[1]) for x in zip(primal_fluid, adjoint_fluid)]
    solid_residuals = [abs(x[0]) + abs(x[1]) for x in zip(primal_solid, adjoint_solid)]
    average = (
        sum([abs(x) for x in fluid_residuals])
        + sum([abs(x) for x in solid_residuals])
    ) / (2 * len(fluid_residuals) + 2 * len(solid_residuals))
    fluid_refinements = []
    solid_refinements = []
